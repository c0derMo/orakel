...
router.get("/get/:tid", async (req, res) => {
    ...
    const amountRounds = Math.ceil(Math.log2(tourney.participants.length));
    ...
    for (let i = 0; i < amountRounds; i++) {
        ...
        for (let j = 0; j < amountMatches; j++) {
            ...
            const matchInfo = tourney.matches.find(e => {
    ...
    if (tourney.doubleElim) {
        const amountLBRounds = (Math.ceil(Math.log2(tourney.participants.length)) - 1) * 2;
        const amountUBMatches = (2 ** (Math.ceil(Math.log2(tourney.participants.length)))) / 2;
            ...
            for (let j = 0; j < amountLBMatches; j++) {
                ...
                if (match.team1.id !== "" && match.team2.id !== "") {
                    ...
                    if (match.team1.name === "BYE") {
                        ...
                        const matchData = tourney.matches.find(e => {
                            return e.id === match.id
                        });
                        if (matchData) {
